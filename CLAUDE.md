...
| Menu navigation | Click hamburger (≡) button then click items by title only | FUSIL uses a custom navigation panel, not a standard MenuBar. `control_type="MenuItem"` not found. Title-only search works after hamburger opens the panel. |
