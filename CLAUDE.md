...
| Hamburger click | `invoke()` on MenuItem `auto_id='MainMenu'` via `descendants()` | `click_input()` fails headless (SetCursorPos). `PostMessage` to parent HWND doesn't route to child control owning the button. UIA `invoke()` on the confirmed auto_id is cursor-free and headless-safe. PostMessage kept as fallback. |
...
| Menu navigation (headless) | Search box `txtSearchMenu` primary; hamburger fallback | In headless Task Scheduler sessions FUSIL's navigation panel never renders so the hamburger UIA element is absent from the tree. The search box (`auto_id='txtSearchMenu'`) is always in the tree. `set_edit_text()` + `PostMessage(WM_KEYDOWN VK_RETURN)` to the search HWND navigates without cursor or display. |
