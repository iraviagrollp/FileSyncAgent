...
| Menu navigation | Hamburger `auto_id='MainMenu'`, top-level items `auto_id=label`, sub-items by title in Desktop popup | Confirmed via diagnostic: hamburger is a MenuItem not a Button; top-level items have auto_id matching their title; sub-menus appear as Desktop popups |
