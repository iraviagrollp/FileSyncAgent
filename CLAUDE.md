...
| Hamburger click | Resolve each spec step explicitly with `exists()` before `click_input()` | Chaining unresolved `WindowSpecification` objects fails; calling `exists()` first resolves the spec — matches the manual test that confirmed working |
