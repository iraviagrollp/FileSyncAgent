...
| Login detection | Count Edit fields: >1 = login screen | Login screen has 4 Edit fields; main screen has fewer (but >1). LOGIN button lookup unreliable for 32-bit app under 64-bit UIA. |
| Login verification | Post-login edit count must DECREASE (not drop to 1) | Main screen also has multiple edit fields; hardcoded threshold of 1 caused false LoginError |
...
| Login failure | Raises `LoginError` → clean exit via finally, no SNS | Wrong credentials are a config problem, not an infra failure; no alert needed |
